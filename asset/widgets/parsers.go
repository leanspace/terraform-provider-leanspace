...
func (series *Series) ToMap() map[string]any {
	...
	seriesMap["filters"] = asset.ParseToMaps(series.Filters)
	return seriesMap
}
...
func (widget *Widget) FromMap(widgetMap map[string]any) error {
	...
	if series, err := asset.ParseFromMaps[Series](widgetMap["series"].([]any)); err != nil {
		return err
	} else {
		widget.Series = series
	}
	if metrics, err := asset.ParseFromMaps[MetricInfo](widgetMap["metrics"].([]any)); err != nil {
		return err
	} else {
	...
	if dashboards, err := asset.ParseFromMaps[DashboardInfo](widgetMap["dashboards"].(*schema.Set).List()); err != nil {
		return err
	} else {
...
func (series *Series) FromMap(seriesMap map[string]any) error {
	...
	if filters, err := asset.ParseFromMaps[Filter](seriesMap["filters"].(*schema.Set).List()); err != nil {
		return err
	} else {
