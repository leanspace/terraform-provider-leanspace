	...
	"attributes": {
		...
		Elem: &schema.Resource{
			Schema: general_objects.DefinitionAttributeSchema(
				[]string{"BINARY", "GEOPOINT", "TUPLE"}, // Attribute types not allowed in attributes
...
var dataSourceFilterSchema = map[string]*schema.Schema{
	"ids": {
		Type:        schema.TypeList,
		Optional:    true,
		Description: "Only returns Record Templates who's id matches one of the provided values.",
		Elem: &schema.Schema{
			Type:         schema.TypeString,
			ValidateFunc: validation.IsUUID,
		},
	...
	"query": {
		Type:        schema.TypeString,
		Optional:    true,
		Description: "Filter by name or description. Search pattern is authorized using % character.",
	},
	"created_by": {
		Type:     schema.TypeList,
		Optional: true,
