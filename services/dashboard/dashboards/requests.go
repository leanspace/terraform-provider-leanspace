...
func nodeAction(action string, node string, dashboardId string, client *provider.Client) error {
...
func attachNode(node string, dashboardId string, client *provider.Client) error {
	return nodeAction("POST", node, dashboardId, client)
}

func detachNode(node string, dashboardId string, client *provider.Client) error {
	return nodeAction("DELETE", node, dashboardId, client)
}

func addWidget(widget WidgetInfo, dashboardId string, client *provider.Client) error {
...
func removeWidget(widget WidgetInfo, dashboardId string, client *provider.Client) error {
	path := fmt.Sprintf("%s/%s/%s/widgets/%s", client.HostURL, DashboardDataType.Path, dashboardId, widget.ID)
...
func (dashboard *Dashboard) PostCreateProcess(client *provider.Client, created any) error {
...
func (dashboard *Dashboard) PostUpdateProcess(client *provider.Client, updated any) error {
	updatedDashboard := updated.(*Dashboard)
	for _, node := range dashboard.NodeIds { // Attach needed nodes
		if !helper.Contains(updatedDashboard.NodeIds, node) {
	...
	for _, node := range updatedDashboard.NodeIds { // Remove extra nodes
		if !helper.Contains(dashboard.NodeIds, node) {
