...
func (stream *Stream) ToMap() map[string]any {
	...
	helper.Logger.Printf("toMap  %s", stream.StreamQueueId)
	streamMap["stream_queue_id"] = stream.StreamQueueId
	return streamMap
}
...
func (stream *Stream) FromMap(streamMap map[string]any) error {
	...
	if streamMap["stream_queue_id"].(string) != "" {
		helper.Logger.Printf("fromMap befor parsing  %s", streamMap["stream_queue_id"].(string))
		stream.StreamQueueId = streamMap["stream_queue_id"].(string)
	}
	helper.Logger.Printf("fromMap after parssing  %s", stream.StreamQueueId)
